...
#if defined(NDEBUG)
# error "BitNetCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf66982770ef8d5419c28d0caaba1a3098d2dc65d7493e88be0893cadaec374a7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitNetCoin: starting difficulty is 1 / 2^12
...
{
    // BitNetCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // BitNetCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // BitNetCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // BitNetCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitNetCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitNetCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xaeddc0abae9e4378fdb3d35adb6790c189a9d70dd211beb45f289062ebcf873c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "July 14/2020 a new coin was born!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0430cba3baa68ee983cbb8c42c780becf1bc5acb9833ffc0ead7f9edbf2d6a62a9b67545f57771bc4636a63fd29dd6a130d8c1dbfb01439638ee10160584b2dd49") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1594761363;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086768150;

        if (fTestNet)
        {
            block.nTime    = 1594761325;
            block.nNonce   = 387672299;
        }

	// Paste to generate Genisis Block
	...
	//end of the paste
        ...
        assert(block.hashMerkleRoot == uint256("0xd1d6ff35c586c66408ed143bc165a1b18b2f602c9006c14d320a962cc34aa1ad"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xda }; // BitNetCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitNetCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitNetCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitNetCoinMiner(CWallet *pwallet)
{
    printf("BitNetCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running BitNetCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitNetCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitNetCoinMiner, pwallet));
}
